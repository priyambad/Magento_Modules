...
function (
	...
	return Component.extend({
        defaults: {
            template: 'Redington_Checkout/additional/reference_info'
        },
		...
		uploadImage: function (fileVal, inputId) {
			if(fileVal){
				...
				if(fileError){
					...
					var formData = $('#checkout_document_upload');
		...
		deleteUploadedRefdoc: function () {
			$("#reference_document").val('');
			var fileTypeVal = "reference_document";
		...
		customAjaxCall: function(fileData){
			...
			$.ajax({
				...
				if(result.success){
					...
					$('#'+blobDocumentContainerId).hide();
					$('#'+blobDocumentDocname).text('');
					$('#'+messageSection).html('<div generated="true" class="mage-error doc-form-val-error" style="display: block;">There was some error during file upload.</div>');
				}
			})
